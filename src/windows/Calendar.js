class Calendar extends Window {
  ...
  toggleCalendarTab(direction) {
    ...
    const borderCovers = [
      elem.find('#borderCover')[0],
      elem.find('#borderCoverVertical')[0],
      elem.find('#borderCoverVertical2')[0],
    ];
    for (let i = 0; i < borderCovers.length; i += 1) {
      const transformX = parseInt(
        getComputedStyle(borderCovers[i])
      ...
      borderCovers[i].style.transform = `translate(${
        transformX + 77 * direction
      }px, ${transformY}px)`;
    }
